...
docker exec -it -u root "$CONTAINERNAME1" wget https://github.com/3dcitydb/3dcitydb/releases/download/v5.1.3/3dcitydb-5.1.3.zip
docker exec -it -u root "$CONTAINERNAME1" unzip 3dcitydb-5.1.3.zip
docker exec -it -u root "$CONTAINERNAME1" rm 3dcitydb-5.1.3.zip
