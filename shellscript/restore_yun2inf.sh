...
# DB BACKUP FILEPATH
#---------------------------------------------------------
echo 'Enter Path of Backup file'
read -p "(default=/path/to/backup.bak): " BACKUPPATH
BACKUPPATH=${BACKUPPATH:-/path/to/backup.bak}
echo
echo 'Enter Path of Role file'
read -p "(default=/path/to/role.sql): " ROLEPATH
ROLEPATH=${ROLEPATH:-/path/to/role.sql}
echo
#---------------------------------------------------------
# POSTGRES
#---------------------------------------------------------
...
#PRINT SETTING
echo '---------------------------------'
echo 'Path of Backup File: ' $BACKUPPATH
echo 'Path of Role File: ' $ROLEPATH
...
docker cp "$ROLEPATH" "$CONTAINERNAME1:/home/postgres/roles.sql"
docker cp "$BACKUPPATH" "$CONTAINERNAME1:/home/postgres/backup.sql"

docker exec -it "$CONTAINERNAME1" psql postgres://$DBUSER@localhost:$DBPORT/$DBNAME -v ON_ERROR_STOP=1 --echo-errors -f /home/postgres/roles.sql
docker exec -it "$CONTAINERNAME1" psql -U "$DBUSER" -d "$DBNAME" -c "SELECT timescaledb_pre_restore();"
docker exec -it "$CONTAINERNAME1" psql postgres://$DBUSER@localhost:$DBPORT/$DBNAME -v ON_ERROR_STOP=1 --echo-errors -f /home/postgres/backup.sql
docker exec -it "$CONTAINERNAME1" psql -U "$DBUSER" -d "$DBNAME" -c "SELECT timescaledb_post_restore();"
