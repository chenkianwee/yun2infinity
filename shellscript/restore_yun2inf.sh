...
#-----------------------------------------------------------
echo '------------------------------------------------------'
echo 'This will help you restore your yun2inf Docker Network'
echo 'Two containers: 1) Database Container 2) FROST-Server Container'
echo '------------------------------------------------------'
#DB BACKUP FILEPATH
...
while [ $TIME -le 180 ]
do
	echo "Configuring database, wait for 180 seconds ... $TIME seconds"
	TIME=$(($TIME + 10))
	sleep 10
done

docker exec -it "$CONTAINERNAME1" psql -U "$DBUSER" -d "postgres" -c "DROP DATABASE $DBNAME;"
docker exec -it "$CONTAINERNAME1" psql -U "$DBUSER" -d "postgres" -c "CREATE DATABASE $DBNAME;"
docker exec -it "$CONTAINERNAME1" psql -U "$DBUSER" -d "$DBNAME" -c 'CREATE EXTENSION IF NOT EXISTS timescaledb CASCADE;'

#Copy the backup file into the container
docker cp "$BACKUPPATH" "$CONTAINERNAME1:./backup.bak"
docker exec -it "$CONTAINERNAME1" psql -U "$DBUSER" -d "$DBNAME" -c "SELECT timescaledb_pre_restore();"
docker exec -it "$CONTAINERNAME1" pg_restore -U "$DBUSER" -Fc -d "$DBNAME" "backup.bak"
